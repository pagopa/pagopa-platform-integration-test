...
tools: [read/readFile, search/fileSearch, web/fetch, vscode/askQuestions]
...
2. **Resolve Gherkin conventions**: read the `## Gherkin Conventions` section in the project's `AGENTS.md`. If absent, search the codebase for existing feature files and ask the user which to follow as a blueprint.
