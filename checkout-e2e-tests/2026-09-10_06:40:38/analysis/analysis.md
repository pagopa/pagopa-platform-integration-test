...
```
### Root cause
The target PSP (`PPAYITR1XXX`) element `#psp-radio-PPAYITR1XXX` failed to load or render within 5 seconds on the UAT PSP list page (`/lista-psp`).

### Category
environment

### Recommended action
Verify whether the Postepay PSP (`PPAYITR1XXX`) is enabled and available in the UAT environment, and consider increasing the locator timeout if load times are slow.
...
```
### Root cause
Back navigation logic failed to return the user to `/dati-pagamento`, unexpectedly sending the user back to the initial page `/inserisci-dati-avviso`.

### Category
application bug

### Recommended action
Inspect routing history and back button event handlers in the frontend checkout application to prevent skipping `/dati-pagamento`.
...
```
### Root cause
The error modal rendered a generic Creditor Institution error message instead of mapping the `PAA_PAGAMENTO_SCONOSCIUTO` code to the specific unknown notice message.

### Category
application bug

### Recommended action
Verify error code mapping logic on the frontend to ensure `PAA_PAGAMENTO_SCONOSCIUTO` displays the expected modal header 'Non riusciamo a trovare l’avviso'.
...
```
### Root cause
The test framework missing the dictionary key `'Modifica metodo di pagamento'` in `BUTTON_SELECTORS` inside `back_navigation.py`.

### Category
test data

### Recommended action
Update the `BUTTON_SELECTORS` dictionary in `src/e2e/checkout/steps/back_navigation.py` to include the key `'Modifica metodo di pagamento'` with its corresponding element selector.
...
```
### Root cause
Linear navigation state was lost during flow navigation, redirecting to `/inserisci-dati-avviso` instead of `/dati-pagamento`.

### Category
application bug

### Recommended action
Investigate checkout session/state persistence across step transitions to ensure previous route history (`/dati-pagamento`) is preserved.
...
```
### Root cause
The element `#psp-radio-PPAYITR1XXX` timed out waiting to appear on `/lista-psp` in the UAT environment.

### Category
environment

### Recommended action
Check UAT environment PSP backend services and verify configuration/availability for `PPAYITR1XXX` across both authenticated and unauthenticated flows.

---

## Common patterns

1. **Routing and Back Navigation Regression**: Failures #2 and #5 both share the identical assertion failure (`Expected page '/dati-pagamento', but found '/inserisci-dati-avviso'`). This indicates a systemic issue in state/history handling during linear backward navigation in the Checkout frontend.
2. **UAT Environment PSP Unavailability**: Failures #1 and #6 both fail due to a 5000 ms locator timeout on `#psp-radio-PPAYITR1XXX` at `/lista-psp`. The Postepay PSP is consistently missing or failing to render in UAT for both authenticated and unauthenticated test suites.
3. **Test Code Definition Drift**: Failure #4 highlights missing configuration keys (`KeyError: 'Modifica metodo di pagamento'`) in step definitions (`BUTTON_SELECTORS`), pointing to missing selector updates in the test suite following UI/copy changes.