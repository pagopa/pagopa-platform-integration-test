...
```
### Root cause
The payment receipt (`receipt-ok`) event with `RT_SEND_SUCCESS` status was not generated or processed correctly during cart checkout.

### Category
application bug

### Recommended action
Investigate the backend service responsible for sending Receipt (RT) notifications for multi-beneficiary carts to ensure events are properly dispatched.
...
```
### Root cause
The single-RPT cart checkout process failed to trigger the expected `receipt-ok` event with `RT_SEND_SUCCESS`.

### Category
application bug

### Recommended action
Check the payment activation and receipt generation flow for single RPT carts with multiple payments to identify why the RT notification event is missing.
...
```
### Root cause
The receipt-ok event verification failed because the expected `RT_SEND_SUCCESS` status event was not recorded for the multi-beneficiary cart.

### Category
application bug

### Recommended action
Verify logs in the WISP integration layer to ensure receipt transmission triggers correctly for multi-RPT and multi-beneficiary scenarios.

---

## Common patterns
All three failures share the exact same assertion error (`AssertionError: There are not events with business process receipt-ok`) stemming from `check_wisp_session_timers_del_and_rts_were_sent`. This indicates a systemic issue across cart payment flows (both single and multi-beneficiary) where the `RT_SEND_SUCCESS` receipt event is not being correctly generated or logged.