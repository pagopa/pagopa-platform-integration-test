...
```
### Root cause
The payment flow did not generate or register the expected `receipt-ok` event (`RT_SEND_SUCCESS`) after redirecting the user to checkout.

### Category
application bug

### Recommended action
Inspect event store services and receipt dispatching backend logs to verify why the RT receipt event failed to publish.
...
```
### Root cause
The single payment process failed to emit the required `receipt-ok` event (`RT_SEND_SUCCESS`) during session verification.

### Category
application bug

### Recommended action
Check the GPD and WISP integration points to ensure receipt processing triggers correctly upon checkout redirection.

---

## Common patterns
Both failures occur at step `check_wisp_session_timers_del_and_rts_were_sent` due to a missing `receipt-ok` business process event. This indicates a common failure mode across both cart and single-payment flows where the asynchronous receipt generation or dispatch pipeline fails to emit the `RT_SEND_SUCCESS` event expected by the verification utilities.