# 1. Utente paga un pagamento singolo con un versamento e nessuna marca da bollo gia esistente in GPD
- **status**: failed
...
```
### Root cause
The test failed to find the expected 'receipt-ok' event with status 'RT_SEND_SUCCESS', indicating the receipt was not successfully generated or processed.

### Category
application bug

### Recommended action
Verify the backend receipt generation service and check logs for errors during the `nodoInviaRPT` payment completion flow.

---

# 2. L'utente paga un carrello con singola RPT con cinque versamenti
- **status**: failed
...
```
### Root cause
The test failed because the `receipt-ok` event with `RT_SEND_SUCCESS` status was never emitted for the cart payment.

### Category
application bug

### Recommended action
Investigate the cart payment flow via `nodoInviaCarrelloRPT` to ensure receipt transmission logic correctly handles multi-transfer carts.

---

# 3. L'utente paga un carrello con due RPT, entrambe senza versamento semplice e con una marca da bollo
- **status**: failed
...
```
### Root cause
Missing `receipt-ok` event with `RT_SEND_SUCCESS` status during cart payment containing stamp duties (marche da bollo).

### Category
application bug

### Recommended action
Check stamp duty validation and receipt dispatching services for cart payments involving multiple RPTs.
...
```
### Root cause
The expected `receipt-ok` event with `RT_SEND_SUCCESS` status was not recorded for the large cart transaction.

### Category
application bug

### Recommended action
Examine timeout or payload limits when processing carts with high numbers of RPTs and transfers.

---

# 5. L'utente paga un carrello con singola RPT con un versamento
- **status**: failed
...
```
### Root cause
Assertion failure due to missing `receipt-ok` event with `RT_SEND_SUCCESS` status on a basic cart scenario.

### Category
application bug

### Recommended action
Review recent deployments affecting the WISP checkout session termination and receipt dispatching pipeline.
...
```
### Root cause
The multi-beneficiary cart payment failed to produce the required `receipt-ok` event with `RT_SEND_SUCCESS` status.

### Category
application bug

### Recommended action
Investigate multi-beneficiary receipt routing logic to ensure `RT_SEND_SUCCESS` events are correctly emitted for all participating creditors.

---

## Common patterns
All 6 test failures share the exact same root cause: the integration test suite fails during the session timer and receipt verification step because the `receipt-ok` event with status `RT_SEND_SUCCESS` was not found. This points to a systemic issue in the environment or application backend regarding receipt generation and event emission across both single and cart/multi-beneficiary payment flows.