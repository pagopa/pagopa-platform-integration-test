...
```
### Root cause
The payment flow completed, but the expected `receipt-ok` event with `RT_SEND_SUCCESS` status was not generated within the monitored timeframe.

### Category
application bug

### Recommended action
Investigate the receipt generation and delivery service (nodo-dei-pagamenti / WISP backend) to ensure `RT_SEND_SUCCESS` events are correctly emitted for multi-RPT carts.

---

# 2. L'utente paga un carrello con singola RPT con due versamenti
- **status**: failed
...
### Recommended action
Check backend logs for errors related to receipt processing and forwarding for carts containing single RPTs with multiple payments.

---

# 3. Utente paga un pagamento singolo con tre versamenti e nessuna marca da bollo
- **status**: failed
...
### Recommended action
Verify why single payments with multiple installments fail to trigger the final `receipt-ok` event notification.

---

# 4. L'utente paga un carrello con singola RPT con due versamenti semplici e una marca da bollo
- **status**: failed
...
File "/opt/hostedtoolcache/Python/3.14.7/x64/lib/python3.14/site-packages/behave/model.py", line 1329, in run
    match.run(runner.context)
    ~~~~~~~~~^^^^^^^^^^^^^^^^
  File "/opt/hostedhubcache/Python/3.14.7/x64/lib/python3.14/site-packages/behave/matchers.py", line 98, in run
    self.func(context, *args, **kwargs)
    ~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "src/integration/wisp/steps/steps.py", line 172, in user_redirected_to_checkout
    steputils.check_wisp_session_timers_del_and_rts_were_sent(context)
...
### Recommended action
Analyze transaction event logs for cart payments involving stamps (marche da bollo) to identify where the RT dispatch mechanism breaks.

---

## Common patterns
All 4 failures share the exact same assertion failure: missing `receipt-ok` business process events with `RT_SEND_SUCCESS` status during the `check_wisp_session_timers_del_and_rts_were_sent` step. This points to a systemic backend issue with receipt generation or event publishing across different payment types (single RPT, multi-RPT, with/without stamps, multiple installments).