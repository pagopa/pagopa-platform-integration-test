...
- Before acting, read `ace-qa-engineer.instructions.md` from `.github/instructions/` using the file-reading tool and cite applied `P-NNN` lessons. Read any family instructions explicitly configured for this agent in `ace/config/project.json.agent_families`.
...
- Do not write traces, proposals, decisions, or playbooks. ACE lessons never override project instructions, safety rules, or domain evidence.
