...
Funzionalità: Creazione di una posizione debitoria per CUP
  ...
  Schema dello scenario: Valorizzazione di più parametri identificativi
    ...
    Allora Il PSP riceve un 200 OK che all'interno riporta il fault code PPT_MULTI_IDENTIFICATIVO_NON_CONSENTITO

    Esempi:
      | organizationFiscalCode | istatCode | catastalCode |
      | 02438750586            | 058091    |              |
      |                        | 058091    | H501         |
      | 02438750586            |           | H501         |
  ...
  Schema dello scenario: Il dato fornito (CF, ISTAT o Catastale) non rispetta il formato sintattico previsto
    ...
    Allora Il PSP riceve un 200 OK che all'interno riporta il fault code PPT_PARAMETRO_IDENTIFICATIVO_ERRATO
  ...
  Schema dello scenario: Mancanza di uno o più parametri obbligatori
    ...
    Allora Il PSP riceve un 200 OK che all'interno riporta il fault code PPT_DATI_OBBLIGATORI_MANCANTI

    Esempi:
      | debtorFiscalCode | debtorFullName | amount |
  ...
  Schema dello scenario: Formato dei campi amount o debtorFiscalCode non valido
    ...
    Allora Il PSP riceve un 200 OK che all'interno riporta il fault code PPT_FORMATO_DATI_INVALIDO

    Esempi:
      | debtorFiscalCode | amount |
      | 00488            |        |
      |                  | 500.00 |
