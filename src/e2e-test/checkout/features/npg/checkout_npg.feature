Feature: Checkout Payment Activation
  ...
  Scenario Outline: Should correctly execute a payment with card configuration "<testingPsp>"
    When The user enters the notice data with a notice code with fiscal code prefix "<fiscalCodePrefix>"
    And The user enters the taxpayer fiscal code "<fiscalCode>"
    ...
    And The user fills in the expiration date "<expirationDate>"
  ...
  Scenario Outline: Error <errorCode> for invalid notice code in range <rangeStart>-<rangeEnd> is shown on verify
    ...
    And The error modal header contains "<expectedHeader>"
    And The error modal body contains "<expectedBody>"
  ...
  Scenario Outline: Error code for unreachable creditor institution is shown on verify
    ...
    And The error modal header contains "<expectedHeader>"
    And The error modal body contains "<expectedBody>"
    And The error code shown contains "<errorCode>"
